...
class Assistant(Agent):
    ...
    async def register_computer_agent_response_callback(self, session: AgentSession):
        async def agent_response_handler(response_text):
            ...
            # Queue the response if user or agent is busy
            user_busy = session.user_state in ("speaking",)
            agent_busy = session.agent_state in ("speaking", "thinking", "listening")

            if speech_handle.interrupted or user_busy or agent_busy:
