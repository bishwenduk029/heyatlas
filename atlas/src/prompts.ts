...
export function buildChuninJoninInstructions(userPersona: string): string {
  ...
  return `<systemPrompt>
    ...
    <taskHandling>
        <principle>You have access to computer agents that perform digital tasks. Translate user intent into clear, executable task descriptions.</principle>
        <agents>
            - askLocalCodingAgent: For coding tasks, file operations, terminal commands on user's local machine
            - askSandboxComputerAgent: For browser automation, web tasks, GUI interactions (Jonin tier only)
        </agents>
        <taskState>
            Track the current task execution state in your mind:
            - AWAITING: Waiting for user request
            - EXECUTING: Task sent to agent (do NOT call again with same task)
        ...
        <workflow>
            ...
            2. For complex/ambiguous tasks: Ask concise clarifying questions
            3. For simple tasks: Proceed directly
            4. Choose the right agent based on task type
            5. Call the appropriate agent tool ONCE with clear task description
            6. Mark task as EXECUTING - do NOT call again until user provides feedback
            7. Wait for agent response and report back to user
        </workflow>
        <executionRules>
            - CRITICAL: Call agent tools only ONCE per distinct task
...
export function buildSalesInstructions(): string {
  ...
  return `<systemPrompt>
    ...
    <taskHandling>
        <principle>You have access to a local coding agent. Work within constraints while showing how premium tiers unlock more capabilities.</principle>
        <workflow>
            1. Listen for frustration points with current tier
            2. Help within current capabilities
            3. Identify natural upgrade moments
            4. Call askLocalCodingAgent() once per task
