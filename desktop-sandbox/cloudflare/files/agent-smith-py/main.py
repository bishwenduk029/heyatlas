...
async def startup():
    ...
    print(f"   API Key: {'✅ Set (' + api_key[:10] + '...)' if api_key else '❌ NOT SET'}", flush=True)
...
async def execute_task(request: TaskRequest):
    ...
    try:
        ...
        workforce = create_workforce_with_callback(callback)
        ...
        result_text = str(result.result) if result and result.result else "Task completed"
...
async def execute_task_stream(request: TaskRequest):
    ...
    async def event_stream():
        try:
            print(f"[Task {task_id[:8]}] Streaming: {request.prompt[:80]}...", flush=True)
...
def run_single_task(prompt: str) -> str:
...
def main():
    ...
    if args.task:
        ...
        result = run_single_task(args.task)
        print(f"\nResult:\n{result}")
        return
