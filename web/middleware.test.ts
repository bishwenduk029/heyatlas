...
describe("authMiddleware", () => {
  ...
  it("should redirect logged-in users from auth pages to /voice", async () => {
    ...
    expect(NextResponse.redirect).toHaveBeenCalledWith(
      new URL("/voice", request.url),
    );
    expect(response?.headers.get("location")).toBe("http://localhost/voice");
  });
  ...
  it("should redirect unauthenticated users from dashboard pages to /login with callbackUrl", async () => {
    ...
    const request = new NextRequest("http://localhost/voice/settings");
    const response = await authMiddleware(request);

    const loginUrl = new URL("/login", request.url);
    loginUrl.searchParams.set("callbackUrl", "/voice/settings");
  ...
  it("should redirect logged-in users from /signup to /voice", async () => {
    (getSessionCookie as jest.Mock).mockReturnValue("some-session-cookie");
  ...
  it("should redirect logged-in users from /auth/sent to /voice", async () => {
    (getSessionCookie as jest.Mock).mockReturnValue("some-session-cookie");
